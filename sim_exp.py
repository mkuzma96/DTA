...
# Parameters
n = 5000; T = 30; p = 20; r = 5; h = 3; k = 2; p_N = 5; burn = 10; tau = 5
# We vary each combination of rank and gamma
rank_size = [int(0.5*p), int(0.25*p)]; rank = rank_size[0] 
gammas = [0, 0.2, 0.4, 0.6, 0.8]; gamma = gammas[0]
# We make 10 runs with np.random.seed = 1,...,10 for each (rank, gamma) combination
seed = 1; np.random.seed(seed)

# The results show prediction loss with standard errors for naive, oracle and
# two ranks, for increasing gamma 
results = {
    'Gamma': gamma,
    'Rank': rank,
    'Optimal params': [],
...
#     a_(t,s) = f_s( l_(t+1-i,j) )                         for i = 1,...,h (lag h), j = 1,...,r  
...
test = test.to(device)
_, _, _, conf_DTA = mod_DTA(train[:,:,(1+k+r):(1+k+r+p)], train[:,:,1:(1+k)], train[:,:,1:(1+k)])
...
y_test = d_test_cf[:,(T-1+s_ahead-tau),0]   
loss3_cf = np.sqrt(np.mean((y_pred-y_test)**2))
results['RMSN'].append(loss3_cf)


file = open("Results_" + str(seed) + "_" + str(rank) + "_" + str(gamma) + ".txt", "w")
